...
class CatiSubDependentDropdownView(formField: FormField) : CatiCustomView(formField) {
    ...
    override fun setQuesLayout() {
        ...
        formField.dependentDropdownKey?.let { key ->
            ...
            if (dropDownData == null) {
                val ii =
                    AppPreference.instance.getDependentDropdown("routeList")!!.dropDownItemList!!.filter { it.value == key }
                dropDownData = DependentDropDownData("subList", ii.get(0).options)
            }
        }
    ...
    private fun addLayoutView(index: Int, label: String): View {
        val view = inflater.inflate(R.layout.cati_global_icon, null)
        view.findViewById<TextView>(R.id.cati_tv).text = label
        textview = view.findViewById<View>(R.id.marketPlaceTextView) as TextView
        imageView = view.findViewById<View>(R.id.mapImageView) as ImageView
    ...
    var dropDownListener: AdapterView.OnItemSelectedListener = object :
    ...
    fun valueChanged(spinnerIndex: Int) {
        if (spinnerIndex < spinnerList.size) {
            ...
            selectedItem.otherDetails?.let { otherDetails ->
                textview!!.visibility=View.VISIBLE
                imageView!!.visibility=View.VISIBLE
                textview!!.text = Html.fromHtml(selectedItem.otherDetails.htmlText)
